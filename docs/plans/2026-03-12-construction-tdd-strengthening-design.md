...
`Generate comprehensive build and test instructions` → `Execute build and full test suite after all units are implemented, then generate reference instructions.`
...
2. debugging 완료 후 build-and-test 재실행
```
...
| `aidlc-systematic-debugging/SKILL.md` | 수정 | 257 → ~275줄 |

...
**파일**: `skills/_shared/patterns/tech-stack-defaults.md`

이 파일은 사용자에게 기술 스택을 질문할 때 선택지를 생성하는 데이터입니다.
...
- 카탈로그에 없는 기술도 사용자가 "직접 입력"으로 선택할 수 있으므로, 모든 기술을 넣을 필요는 없습니다
...
| **자유 입력 보장** | 모든 질문에 직접 입력 옵션 | 조정 불필요 (고정 원칙) |
...
프로젝트 시작
  → aidlc-using-devflow 호출
    → workspace-detection이 CLAUDE.md 읽음
      → 기술 스택이 명시되어 있으면 tech-stack 질문 스킵
      → 미명시 항목만 질문
...
- tech-stack-defaults.md 카탈로그에서 선택지를 생성하여 질문

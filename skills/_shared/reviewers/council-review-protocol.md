...
2. 논리 오류 — 조건문 누락, 경계값 처리, race condition
3. 테스트 갭 — 테스트되지 않은 경로, edge case 미검증
4. 회귀 리스크 — 기존 기능에 영향을 줄 수 있는 변경
...
외부 AI가 1개만 있을 때, 해당 AI에게 두 관점을 모두 포함한 프롬프트를 전달한다.

```
## 리뷰 관점 (통합 — 두 관점 모두 수행)

아래 파일을 읽고 **두 가지 관점**에서 리뷰하세요.
각 관점별로 섹션을 나누어 결과를 작성하세요.

### 관점 1: [Codex 원래 관점명]
[Codex 프롬프트의 검토 항목 전체]

...
Agent tool (general-purpose):
  ...
  prompt: |
    ...
    **invoke_mode별 필수 섹션:**
    ...
    ## 5. 수정 시 영향도 검증 (기존 스킬 수정인 경우)

    | 항목 | 검증 기준 |
    |------|----------|
    | 호출 경로 | 이 스킬을 호출하는 오케스트레이터/스킬에서 기대 동작이 유지되는가 |
    | description 키워드 | 스킬의 핵심 특성 키워드(TDD, Review 등)가 의도치 않게 삭제되지 않았는가 |
    | output_path | 산출물 경로 변경 시, 하류 스킬이 해당 경로를 참조하지 않는가 |
    | 메타 태그 | @gate/@step 변경 시 `tests/run-all.sh` 통과 여부 |
